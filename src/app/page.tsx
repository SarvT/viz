...
export default function Home() {
  const initialMaze = Array(10).fill(null).map(() => Array(10).fill(false))
  const [selectedAlgorithm, setSelectedAlgorithm] = useState<string>("binaryTree")
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      {selectedAlgorithm === "binaryTree" && <BinaryTreeVisualizer data={inputData} />}
      {selectedAlgorithm === "pathfinding" && <PathfindingVisualizer initialMaze={initialMaze} />}    </main>
  )
}
